...
short explanation with a command you can run and the output it produces, so you
can copy the command, run it against your own checkout, and see the same thing.
...
| 6 | [Typed errors](06-errors.md) | A small closed set of typed errors you classify with `errors.Is`/`errors.As`. | `errors` | core |
...
2. **Priority** — the feature that sets gostructor apart from a plain unmarshaller.
3. **Sources** / **Field types** — real backends and the full type range.
4. **Hooks** / **Typed errors** — validation, transformation, failure modes.
