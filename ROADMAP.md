...
type Config struct {
    ...
    DBURL   string        `cfg:"dbURL,env:DB_URL,json:db.url"`
    Timeout time.Duration `cfg:"timeout" gos:"default:30s"`
}
```

(The two-tag model the engine actually uses: `cfg` routes/names a field — a base
name plus optional `source:override` keys — and `gos` carries behavior
(`default`, `sep`, `secret`, `optional`). Priority is the source **slice order**
passed to `WithSources`, not a per-field tag; the generated `Fill` honors the
exact same order.)
...
// Fill resolves every field of Config from the given sources, in source
// slice order, without runtime reflection. Semantics match
...
- Conversions reuse the same `internal/convert` logic through new
  reflection-free typed entry points (e.g. `convert.Int64(any) (int64, error)`,
  `convert.Duration(any)`), which share the exact overflow/integral-float/base-10
  checks the reflective `toInt64`/… already enforce — so the lossless/strict
  guarantees and `*ConvertError` taxonomy are identical, byte-for-byte, without a
  second copy of the rules to drift. (Today's `convert.Value` is `reflect.Value`
  -based; adding these `any`-typed siblings is a prerequisite of the generator.)
...
**1. Kill tag duplication — the `cfg` base name already is the convention.**
The old pain was one key per source to keep in sync. The shipped two-tag model
already fixes most of it: a `cfg` tag carries **one base name** plus optional
`source:override` keys, and each source derives its own key from that base via
its naming strategy (env → `SCREAMING_SNAKE` through `ScreamingSnake`, file
sources → the base as-written through `Identity`). So `cfg:"databasePort"` alone
resolves as `DATABASE_PORT` in env and `databasePort` in JSON/YAML — no tag per
source — and you write an explicit `env:…`/`json:…` override only for the one
source whose real key deviates:

```go
type Config struct {
    DatabasePort int                                 // env DATABASE_PORT, json/yaml databasePort — no override
    LegacyDSN    string `cfg:"dsn,env:OLD_DB_URL"`   // exception: override only the source that breaks the convention
    APIKey       string `cfg:"apiKey" gos:"secret,required"`
}
```

What codegen uniquely adds on top of what the reflective engine already does:

- A file-level `//gostructor:naming=<snake_case|kebab|screaming_snake|…>`
  directive to **change the per-source derivation** (e.g. make JSON/YAML keys
  `snake_case` instead of the literal base name). Today the strategies are fixed
  per source; the directive lets a project pick its house style once.
- The generator **bakes the resolved key as a string literal** into `Fill`, so
  the `ScreamingSnake` name-mangling runs at `go generate` time, not on every
  fill — the runtime does a plain map/env lookup with a constant key.
- Reflective-engine parity: the same naming override is exposed as an opt-in
  `WithNaming(...)` option, so behavior matches whether or not you ran
  `go generate`.
...
run." Add a `gos:"required"` flag (a bare flag alongside `secret`/`optional`).
The generated `Fill` emits a direct post-resolution `if` per required field: if
the field is still the zero value for its type after every source (and default)
was tried, return a precise error naming the field and the sources checked:

```go
fmt.Errorf("gostructor: missing required config field %q (checked: yaml, env)", "DB")
```
...
`gos:"required"` checks pass. `cfg, err := LoadConfig()` with `err == nil` then
...
straight from the struct's fields, tags, `gos:"default:…"` values, and
`gos:"desc:…"` text. Because it regenerates from the single source of truth, a test can
